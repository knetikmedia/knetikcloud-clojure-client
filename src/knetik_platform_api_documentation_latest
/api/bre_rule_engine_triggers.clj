...
(defn get-bre-triggers-with-http-info
  ...
  ([{:keys [filter-system filter-category filter-name size page ]}]
   (call-api "/bre/triggers" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"filter_system" filter-system "filter_category" filter-category "filter_name" filter-name "size" size "page" page }
              :form-params   {}
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["OAuth2"]})))
