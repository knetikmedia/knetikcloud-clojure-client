...
(defn get-bre-event-logs-with-http-info
  ...
  ([{:keys [filter-start-date filter-event-name filter-event-id size page order ]}]
   (call-api "/bre/logs/event-log" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"filter_start_date" filter-start-date "filter_event_name" filter-event-name "filter_event_id" filter-event-id "size" size "page" page "order" order }
              :form-params   {}
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    ["OAuth2"]})))
