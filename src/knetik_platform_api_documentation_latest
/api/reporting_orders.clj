...
(defn get-invoice-reports-with-http-info
  ...
  ([currency-code {:keys [granularity filter-payment-status filter-fulfillment-status start-date end-date ]}]
   (call-api "/reporting/orders/count/{currency_code}" :get
             {:path-params   {"currency_code" currency-code }
              :header-params {}
              :query-params  {"granularity" granularity "filter_payment_status" filter-payment-status "filter_fulfillment_status" filter-fulfillment-status "start_date" start-date "end_date" end-date }
              :form-params   {}
