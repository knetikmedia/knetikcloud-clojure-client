...
(defn get-users-with-http-info
  ...
  ([{:keys [filter-displayname filter-email filter-firstname filter-fullname filter-lastname filter-username filter-tag filter-group filter-role filter-search size page order ]}]
   (call-api "/users" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"filter_displayname" filter-displayname "filter_email" filter-email "filter_firstname" filter-firstname "filter_fullname" filter-fullname "filter_lastname" filter-lastname "filter_username" filter-username "filter_tag" filter-tag "filter_group" filter-group "filter_role" filter-role "filter_search" filter-search "size" size "page" page "order" order }
              :form-params   {}
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    []})))
