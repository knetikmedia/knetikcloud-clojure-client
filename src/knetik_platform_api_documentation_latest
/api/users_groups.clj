...
(defn add-member-to-group
  "Adds a new member to the group"
  [unique-name username ]
  (:data (add-member-to-group-with-http-info unique-name username)))
...
(defn update-groups-with-http-info
  ...
  ([{:keys [filter-name filter-unique-name filter-parent size page order ]}]
   (call-api "/users/groups" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"filter_name" filter-name "filter_unique_name" filter-unique-name "filter_parent" filter-parent "size" size "page" page "order" order }
              :form-params   {}
              :content-types ["application/json"]
              :accepts       ["application/json"]
              :auth-names    []})))
